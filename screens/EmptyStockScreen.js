...
export default class EmptyStockScreen extends React.Component{
  	constructor(props) {
		super(props);
		this.state = {
			'data': '',
	...
	async componentDidMount(){
		...
		let url = 'http://monitore.yogya.com/getEmptyStock.php?branch='+store_init.value;
  	...
  	goToScreen = async (sku) => {
    	this.props.navigation.navigate('ProductDetail' , { 'sku': sku, 'screen': 'oos'});
  	};  
  	...
  	render() {
		...
		return (
				<Row key={item.article_code}>
				<Col style={styles.column} >
					<TouchableOpacity onPress={() => this.goToScreen(item.sku) }>
						<Text style={styles.value}>{Math.round(item.stock)} pcs</Text>
		...
		return (
			...
			//                   </View>
			//                 }
			//               keyExtractor={({article_code}, index) => article_code}
			//             />
			//           </View>
			//       </Content>
			//     </ScrollView> 
